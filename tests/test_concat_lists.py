...
@pytest.mark.asyncio
async def test_concat_lists(context, messages, httpx_mock):
    ...
    await concat_lists.concat_lists(
        Artifact(
            local_id="#abcd",
...
@pytest.mark.asyncio
async def test_dont_concat_things_that_are_not_lists(context, messages, httpx_mock):
