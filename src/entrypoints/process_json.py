...
description = """\
Reads and processes JSON artifacts, for example:
...
Provide an artifact and describe how its JSON data should be filtered or transformed. Both JSON objects and JSON lists
are accepted. Under the hood, the data are transformed using JQ (https://jqlang.org/), a lightweight open-source JSON
processing tool.
"""


class Parameters(BaseModel):
    artifacts: list[Artifact] = Field(description="The JSON data to process", min_length=1)
...
# async def convert_to_json(artifact: Artifact) -> AsyncIterator[Message]:
#     match artifact.mimetype:
#         case "application/json":
#             return artifact
#     process.log(summary="Converting artifact to JSON", description="Retrieving artifact data")

async def run(context: ResponseContext, request: str, params: Parameters):
    async with context.begin_process("Processing data") as process:
        process: IChatBioAgentProcess

        await process.log("Retrieving artifact data")
        source_artifact = params.artifacts[0]
        source_content = NONE

        async with AsyncClient(follow_redirects=True) as http:
            for url in source_artifact.get_urls():
                await process.log(f"Retrieving artifact {source_artifact.local_id} content from {url}")
        ...
        try:
            jq_query, output_description, output_dict = await _generate_jq_query(request, schema, source_content,
                                                                                 source_artifact)
        ...
        await process.log(f"Query generated {output_size_in_bytes} bytes of data")
...
class JsonArtifactQuery(BaseModel):
    ...
    jq_query_string: str = Field(description="A JQ query string to process the json artifact.")
    output_description: str = Field(
        description="A concise characterization of the data that the query will retrieve",
        examples=["List of collectors of Rattus rattus records in iDigBio",
    ...
    def validate_jq_query_string(cls, query):
        ...
        try:
            ...
            raise ValueError(f"Failed to execute JQ query {query} on provided content", e)

        return query


async def _generate_jq_query(request: str, schema: dict, source_content: dict | list, source_artifact: Artifact) -> (
        str, str, dict):
    ...
    messages = [
        ...
        {"role": "user", "content": f"The data have the following metadata: \"{source_meta}\""},
        {"role": "user",
         "content": "The JSON data to be processed have the JSON Schema definition that follows. Assume that fields "
                    ...
                    json.dumps(schema).replace(": {\"type\": \"string\"}", "")},  # Save some tokens
        {"role": "user",
